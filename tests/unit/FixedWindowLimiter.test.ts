...
describe('FixedWindowLimiter', () => {
  ...
  describe('wall-clock alignment', () => {
    it('resetAt aligns to window boundary', async () => {
      ...
      const result = await limiter.check('limiterx:user', 'user')
      expect(result.resetAt.getTime()).toBe(expectedWindowEnd)
    })

    it('two requests in the same window share the same resetAt', async () => {
      const r1 = await limiter.check('limiterx:user', 'user')
      const r2 = await limiter.check('limiterx:user', 'user')
  ...
  describe('allow/deny transitions', () => {
    ...
    it('denies the 6th request (exceeds max)', async () => {
      for (let i = 0; i < max; i++) {
        await limiter.check('limiterx:user', 'user')
      }
      const result = await limiter.check('limiterx:user', 'user')
  ...
  describe('result fields', () => {
    ...
    it('key reflects the displayKey passed in', async () => {
      const result = await limiter.check('limiterx:user-123', 'user-123')
  ...
  describe('window reset', () => {
    it('resets counter after the window elapses', async () => {
      ...
      // Should be denied
      const denied = await limiter.check('limiterx:user', 'user')
      ...
      const allowed = await limiter.check('limiterx:user', 'user')
  ...
  describe('independent keys', () => {
    it('different keys are tracked independently', async () => {
      for (let i = 0; i < max; i++) {
        await limiter.check('limiterx:userA', 'userA')
      }
      const deniedA = await limiter.check('limiterx:userA', 'userA')
      expect(deniedA.allowed).toBe(false)

      // userB should still be allowed
      const allowedB = await limiter.check('limiterx:userB', 'userB')
