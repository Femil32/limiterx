...
- **CI** — `publish` job now auto-creates a GitHub Release with generated notes on every `v*` tag push (`gh release create --generate-notes`)
- **CI** — added `contents: write` per-job permission to `publish` job so `gh release create` works without elevated repo permissions
...
- CI: set `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` to silence Node.js 20 deprecation warning in GitHub Actions (deadline: June 2026)
